...
class RAM(nn.Module):
    def __init__(self,
                 ...
                 threshold=0.68,
                 delete_tag_index=[],
                 tag_list=f'{CONFIG_PATH}/data/ram_tag_list_vad.txt',
        ...
        """
        super().__init__()
        print("vit_grad_ckpt",vit_grad_ckpt)
        print("vit_ckpt_layer",vit_ckpt_layer)
        ...
        if vit == 'swin_b':
            ...
            vision_config_path = f'{CONFIG_PATH}/configs/swin/config_swinL_336.json'
            vision_config = read_json(vision_config_path)
            assert image_size == vision_config['image_res']
            vision_width = vision_config['vision_width']
            self.visual_encoder = CLIPVisionTower('openai/clip-vit-large-patch14-336', args=None)
        ...
        self.tag_proj = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(1024, 1536)),
            ("GLUE", nn.GELU()),
            ("linear2", nn.Linear(1536, 1536))
        ]))
    ...
    def forward(self, image, caption, image_tag, parse_tag, clip_feature):
        ...
        image_features = self.tag_proj(image_features)
        ...
        loss_tag = self.tagging_loss_function(logits, image_tag.to(image.device))
    ...
    def generate_tag(self,
        ...
        if image_features is None:
            image_features = self.visual_encoder(image.float())
        
        image_features_tag = self.tag_proj(image_features) 
        
        image_embeds = self.image_proj(image_features_tag)
        image_atts = torch.ones(image_embeds.size()[:-1],
                                dtype=torch.long).to(image.device)
        ...
        if self.llava_tokenizer:
            tag_ouput_embedding = self.llava_tokenizer(tag_output).input_ids 
            return image_features, torch.tensor(tag_ouput_embedding).to(image.device), tag_output
        else:
