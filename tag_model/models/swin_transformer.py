...
def interpolate_relative_pos_embed(rel_pos_bias, dst_num_pos, param_name=''): # 对输入的相对位置偏置进行插值，以适应不同大小的输入序列
    # 在模型结构改变时，例如改变输入序列长度或位置数时，确保相对位置偏置表能正确适应新的结构，从而保持模型的性能。
