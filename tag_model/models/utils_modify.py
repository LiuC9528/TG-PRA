...
def init_tokenizer(text_encoder_type='bert-base-uncased'):
    ...
    tokenizer = BertTokenizer.from_pretrained(r"/home/bit118/Liucheng/modelDir/bert-base-uncased")
...
def load_checkpoint_swinbase(model, url_or_filename, kwargs):
    ...
    if "fc.W" in state_dict.keys():
        state_dict["fc.W"] = torch.nn.Parameter(torch.Tensor(1, Tags_lists_len, 768))
        state_dict["fc.b"]= nn.Parameter(torch.Tensor(1, Tags_lists_len))
        
        stdv = 1. / math.sqrt(state_dict["fc.W"].size(2))
        for i in range(Tags_lists_len):
            state_dict["fc.W"][0][i].data.uniform_(-stdv, stdv)
        for i in range(Tags_lists_len):
            state_dict["fc.b"][0][i].data.uniform_(-stdv, stdv)

    idx_slice = range(0, Ori_tags_len)
    
    overlap_label_embed_weight = state_dict["label_embed.weight"][idx_slice,:]
    new_label_embed_weight = torch.nn.Parameter(torch.Tensor(Tags_lists_len - Ori_tags_len, 768))
    nn.init.trunc_normal_(new_label_embed_weight, std=.02)
    state_dict["label_embed.weight"] = torch.cat((overlap_label_embed_weight, new_label_embed_weight), 0)
